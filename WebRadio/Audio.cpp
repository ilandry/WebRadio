...
class ObjectPool
{
    ...
    ~ObjectPool()
    {
        std::for_each(_used.begin(), _used.end(), 
                [](T * packet)
                {
                    FreeFunction(packet);
                    delete packet;
    ...
    T * acquire()
    {
        ...
        else
        {
            ...
            LOG << _used.size() << " object created, consider allocating more to pool than " << _size;
        }
        return packet;
    }
...
void audioCallback(void *userData, std::uint8_t * stream, int len)
{
    ...
    boost::fibers::fiber pullData([dataChannel, stream, len]()
            {
            float data;
            while (boost::fibers::channel_op_status::success == dataChannel->pop(data) 
...
CustomAvioContext::CustomAvioContext(std::string input)
    ...
    , _pos(0)
      ...
      , _context(avio_alloc_context(
                  ...
                  0,  
                  this,
                  &CustomAvioContext::read,
                  // write fct ptr 
                  nullptr,
                  &CustomAvioContext::seek)
              )
{}
...
int CustomAvioContext::read(void* userData, std::uint8_t* buffer, int bufferSize)
{
    if (userData == nullptr)
    {
    ...
    const int count = std::min(static_cast<int>(ctx->_data.size() - ctx->_pos), bufferSize); 
...
::AVIOContext * CustomAvioContext::getContext()
{
    return _context;
}
...
{
    int err = avformat_open_input(&_formatCtx, "no file, we look in memory with custom context", nullptr, nullptr);
    if (err < 0)
    {
    ...
    _idxAudioStream = ::av_find_best_stream(_formatCtx, AVMEDIA_TYPE_AUDIO, -1, -1, &_codec, 0);
...
bool FFmpegWrapper::isInit() const
{
    return _isInit;
}
...
void FFmpegWrapper::bufferData(PacketChannel & packetChannel, DataChannel & dataChannel)
{
    ...
    while (boost::fibers::channel_op_status::success == packetChannel.pop(packet))
    {
        ...
        while (read < packet->size)
        {
            ...
            const int retDecode = ::avcodec_decode_audio4(_codecCtx, frame, &hasFrame, packet);
            if (retDecode < 0)
            {
